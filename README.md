...
为符合微信「游戏内容须走小游戏类目」的审核口径,小程序版把产品形态包装成**课程**而非游戏(逻辑层不变,仅展示层):

- **首页 = 课程目录**(`pages/index`):单元 → 课时列表 + 总进度条;原抖音式整屏滑动保留为二级入口**「连续学习」**(`pages/feed`)
- **星级 → 掌握度**:存档仍是 1~3 星,展示层映射为 已掌握 / 基本掌握 / 待巩固(`src/utils/mastery.ts`)
- **课时页四段结构**:本课要点(讲解卡)→ 示范(棋盘标注)→ 随堂练习(做题)→ **本课小结**(掌握度 + 本课术语回顾 + 下一课)
- **新增教学页面**:`pages/glossary` 术语词典(搜索 + 全部大白话释义)、`pages/report` 学习报告(单元掌握度统计 + 学习建议)
- **人机对弈页已删除**(游戏属性最强、审核风险最高;web 版 `PlayView` 仍在 main 分支,过审后如需可作为"实战练习"加回)
- 全局文案课程化:关卡→课时、闯关→课程、过关→完成本课;应用名建议「围棋入门课」,类目报 教育>在线教育
